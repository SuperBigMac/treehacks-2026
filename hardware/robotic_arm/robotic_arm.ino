...
void setup()
{
  ...
  dxl.begin(1000000);              // from your scan
  dxl.setPortProtocolVersion(2.0); // XL330 uses protocol 2.0

  initServo(SERVO_X_ID);
  initServo(SERVO_Y_ID);
  
  DEBUG_SERIAL.println("=================================== ");
  DEBUG_SERIAL.println("Commands: x <deg>   or   y <deg>");
  DEBUG_SERIAL.println("x range: [-180, 180]   y range: [-20, 90]");
  DEBUG_SERIAL.println("Example:  x 180");
  DEBUG_SERIAL.println("=================================== ");
}
...
void loop()
{
  ...
  line.trim();                 // removes spaces and '\r'
